...
CTrapSleep::CTrapSleep(int x, int z, int trap_type) :CTrap(trap_type), C3DObj(C3DObj::TYPE_TRAP)
{
...
CTrapSleep::~CTrapSleep()
{

}

void CTrapSleep::Initialize(int x, int z, int trap_type)
{
	alive = true;
	m_Mapz = z;
	m_Mapx = x;
...
void CTrapSleep::Finalize(void)
{

}

void CTrapSleep::Update(void)
{

}

void CTrapSleep::Draw(void)
{
	// ワールド座標変換行列の作成と設定
	D3DXMATRIX mtxWorld;
	...
	if (alive && m_WanaCheck)
	{
		...
		CBilboard::BilBoard3_Draw(CTexture::TEX_WARPTRAP_ICON, m_Position);
	}
}

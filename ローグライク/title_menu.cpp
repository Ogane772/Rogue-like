...
void CTitleMenu::TitleMenu_Update(void)
{
	...
	if (!(JoyDevice_IsTrigger(CONTROLLER::A_BUTTON)) && !(JoyDevice_IsTrigger(CONTROLLER::B_BUTTON)) && !(JoyDevice_IsCrossTrigger(UP_BUTTON)) && !(JoyDevice_IsCrossTrigger(DOWN_BUTTON)) && (js.lX >= -2 && js.lX <= 2 && js.lY >= -2 && js.lY <= 2))
	{
		trigger = false;
	}
	if (!g_bIsFade)

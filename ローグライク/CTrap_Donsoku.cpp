...
CTrapDonsoku::CTrapDonsoku(int x, int z, int trap_type) :CTrap(trap_type), C3DObj(C3DObj::TYPE_TRAP)
{
...
CTrapDonsoku::~CTrapDonsoku()
{

}

void CTrapDonsoku::Initialize(int x, int z, int trap_type)
{
	alive = true;
	m_Mapz = z;
	m_Mapx = x;
...
void CTrapDonsoku::Finalize(void)
{

}

void CTrapDonsoku::Update(void)
{

}

void CTrapDonsoku::Draw(void)
{
	// ワールド座標変換行列の作成と設定
	D3DXMATRIX mtxWorld;
	...
	if (alive && m_WanaCheck)
	{
		...
		CBilboard::BilBoard3_Draw(CTexture::TEX_DONSOKUTRAP_ICON, m_Position);
	}
}

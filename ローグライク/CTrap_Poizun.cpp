...
CTrapPoizun::CTrapPoizun(int x, int z, int trap_type) :CTrap(trap_type), C3DObj(C3DObj::TYPE_TRAP)
{
...
CTrapPoizun::~CTrapPoizun()
{

}

void CTrapPoizun::Initialize(int x, int z, int trap_type)
{
	alive = true;
	m_Mapz = z;
	m_Mapx = x;
...
void CTrapPoizun::Finalize(void)
{

}

void CTrapPoizun::Update(void)
{

}

void CTrapPoizun::Draw(void)
{
	// ワールド座標変換行列の作成と設定
	D3DXMATRIX mtxWorld;
	...
	if (alive && m_WanaCheck)
	{
		...
		CBilboard::BilBoard3_Draw(CTexture::TEX_POIZUNTRAP_ICON, m_Position);
	}
}
